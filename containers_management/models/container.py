...
class Container(models.Model):
    ...
    type = fields.Selection(
        [('flat_dumpster15', 'Flat Dumpster 1,5'),
         ...
         ('trapezoidal4', 'Trapezoidal 4'), ('trapezoidal6', 'Trapezoidal 6'),
         ('trapezoidal8', 'Trapezoidal 8'), ('other', 'Other')],
        'Type', required=True, default='flat_dumpster9')
    shape = fields.Selection([('opened', 'Opened'), ('closed', 'Closed')],
                             'Shape', required=True, default='opened')
    dimensions = fields.Char('Dimensions', size=32)
    capacity = fields.Float('Capacity (m³)',
                            digits=dp.get_precision('Product UoM'))
    note = fields.Text('Observations')
    active = fields.Boolean('Active', default=True)
    history_ids = fields.One2many('container.move', 'container_id', 'History',
                                  readonly=True)
    ...
    def write(self, vals):
        ...
        if vals.get('situation_id', False) and not \
                self._context.get('no_create_moves', False):
