...
def conv_ascii(text):
    """Convierte vocales acentuadas, € y ç a sus caracteres equivalentes ASCII"""
    old_chars = ['à','è','ì','ò','ù','ä','ë','ï','ö','ü','â','ê','î','ô','û','À','È','Ì','Ò','Ù','Ä','Ë','Ï','Ö','Ü','Â','Ê','Î','Ô','Û','ç','Ç','ª','º','€']
    new_chars = ['a','e','i','o','u','a','e','i','o','u','a','e','i','o','u','A','E','I','O','U','A','E','I','O','U','A','E','I','O','U','c','C','a','o','EUR']
...
def _create_facturae_file(self, cr, uid, data, context):
    ...
    def _parties_facturae(cr, context):
        ...
        if not invoice.address_tramit_id and not contact_partner_address_obj.sef:
            log.add(_('User error:\n\nThis invoice does not have a tramit address or sef code in contact address.'))
            raise log
        tramit_partner_address_obj = invoice.address_tramit_id
        ...
        if invoice_partner_address_obj.dir3 or contact_partner_address_obj.sef:
            texto += "<AdministrativeCentres>"
            administrative = True

        def create_administrative_centres(address, code, sef=False):
            texto = ""
            if administrative:
                 texto += "<AdministrativeCentre>"
                 if not address.dir3 and not not address.sef:
                    log.add(_('User error:\n\nNo dir3 or sef codes on address. Code %s.') % code)
                    raise log
                 texto += "<CentreCode>" + (sef and address.sef or address.dir3) + "</CentreCode>"
        ...
        if administrative:
            if not tramit_partner_address_obj:
                texto += create_administrative_centres(contact_partner_address_obj, "01", sef=True) #Único
            else:
                texto += create_administrative_centres(invoice_partner_address_obj, "01") # Oficina contable
                texto += create_administrative_centres(contact_partner_address_obj, "02") # Órgano Gestor
                texto += create_administrative_centres(tramit_partner_address_obj, "03") # Unidad tramitadora
            texto += "</AdministrativeCentres>"
    ...
    def _invoices_facturae():
        ...
        texto += '<InvoiceNumber>' + (len(number) > 1 and u"/".join(number[1:]) or invoice.number) + '</InvoiceNumber>'
        texto += '<InvoiceSeriesCode>' + (len(number) > 1 and number[0] or "") + '</InvoiceSeriesCode>'
    ...
    if not invoice_ids or len(invoice_ids) > 1:
        raise wizard.except_wizard(_('Error !'), _('Only can select one invoice to export'))

    invoice = pool.get('account.invoice').browse(cr, uid, invoice_ids[0], context)
    contador = 1
    lines_issued = []
    ...
    return {'note':log(), 'reference':invoice.id, 'facturae':file, 'facturae_fname':file_name, 'state':'succeeded'}
