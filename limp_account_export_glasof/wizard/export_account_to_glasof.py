...
class ExportAccountToGlasof(models.TransientModel):
    ...
    state = fields.Selection([('no_export', 'No Export'),('export', 'Exported')],'State',readonly=True, default='no_export')
    account_length = fields.Integer('Account length', required=True, default=11)
    ...
    def format_normal_account(self, account, delegation=False, department=False):
        ...
        if len(account.code) <= total_len:
            if account.code.startswith('6') or account.code.startswith('7'):
                new_account_code = account.code[:parent_len] + ((delegation and not nd) and delegation.code or "") + account.code[parent_len:].rjust(rjust_len, "0") + ((department and not nd) and department.code or "")
            else:
                new_account_code = account.code[:parent_len] + "".rjust(total_len - len(account.code), "0") + account.code[parent_len:]
    ...
    def export_account_moves(self):
        ...
        objects = self.env['account.move'].browse(self._context.get('active_ids',[]))
        move_lines=[]
        acc_numbers = {}
        for move in objects:
            for line in move.line_ids:
                if line.partner_id and (line.account_id.code.startswith('40') or line.account_id.code.startswith('41') or line.account_id.code.startswith('43')):
                    if not line.partner_id.ref:
                        ...
                        acc_numbers[line.id] = self.format_acc_number(line.account_id.code,line.partner_id.ref)
                else:
                    acc_numbers[line.id] = self.format_normal_account(line.account_id, line.delegation_id, line.department_id)
        ...
        doc = tmp.render_unicode(objects=move_lines, acc_numbers = acc_numbers, env=self.env, formatAccount=self.format_acc_number).encode('utf-8','replace')

        partners = set()
        acc_numbers = {}
        for move in objects:
            for line in move.line_ids:
                receiv_acc = line.partner_id.property_account_receivable_id.code
                pay_acc = line.partner_id.property_account_payable_id.code
                move_acc = line.account_id.code
                if move_acc in [pay_acc,receiv_acc]:
                    if not line.partner_id.ref:
                        raise UserError(_('The partner %s has not field ref. Please fill this field and try again.') % line.partner_id.name)

                    acc_numbers[line.partner_id.id] = self.format_acc_number(move_acc,line.partner_id.ref)
        ...
        doc2 = tmp.render_unicode(objects=partners,acc_numbers = acc_numbers, env=self.env).encode('utf-8','replace')

        xdiario_base64 = base64.b64encode(doc)
        xsubcta_base64 = base64.b64encode(doc2)
        self.write({ 'file_xdiario' : xdiario_base64 ,'file_xsubcta' : xsubcta_base64, 'state' : 'export', 'xdiario_name': "xDiario.txt", 'xsubcta_name': "xSubCta.txt" })
