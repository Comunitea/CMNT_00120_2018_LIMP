...
class DistributionEffectiveCosts(models.TransientModel):
    ...
    year = fields.Integer('Year', required=True,
                          default=lambda r: int(time.strftime("%Y")))
    ...
    def distribute_costs(self):
        ...
        first_day, last_day = calendar.monthrange(
            int(year), int(month))  # obtenemos el último días del mes
        ...
        for employee in self.env['hr.employee'].search([]):
            ...
            for timesheet in timesheets:
                if timesheet.analytic_id:
                    duplicated_lines = self.env['account.analytic.line'].search(
                        [('timesheet_id', '=', timesheet.id),
                         ('account_id', '=', timesheet.analytic_id.id),
                         ('name', '=',
                          ustr(self.name) + u" (effective)/ " + month +
                          u"/" + year + u"/ " +
                          employee.name)])  # borramos duplicados
                    if duplicated_lines:
                    ...
                    if amount:
                        vals = {
                            'amount': -(amount),
                            'name': ustr(self.name) + u" (effective)/ " +
                            month + u"/" + year + u"/ " + employee.name,
                            'tag_ids': [(4, sueldos_tag[0].id)],
                            'timesheet_id': timesheet.id,
                            'account_id': timesheet.analytic_id.id,
                            'general_account_id':
                                general_account_id[0].id,
                            'date':
                                year + "/" + month + "/" +
                                str(calendar.monthrange(int(year),
                                                        int(month))[1]),
                            'department_id':
                                timesheet.department_id and
                                timesheet.department_id.id or
                                (timesheet.analytic_id.department_id and
                                 timesheet.analytic_id.department_id.id or
                                 False),
                            'delegation_id':
                                timesheet.delegation_id and
                                timesheet.delegation_id.id or
                                (timesheet.analytic_id.delegation_id and
                                 timesheet.analytic_id.delegation_id.id or
                                 False),
                            'manager_id':
                                timesheet.responsible_id and
