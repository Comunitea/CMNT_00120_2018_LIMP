...
class DistributionCostsImport(models.TransientModel):
    ...
    def import_distribution_costs(self):
        ...
        general_account_id_suelsala = self.env["account.account"].search(
            [("code", "=", "64000000")]
        )
        visited_rows = []
        for line in range(1, sh.nrows):  # nos recorremos el fichero
            ...
            if (
                ...
                record = {
                    ...
                    "ss_company": row[10]
                    and float_format(row[10])
                    or 0.0,  # total seguridad social
                    "total_accrued": row[12]
                    and float_format(row[12])
                    or 0.0,  # total bruto
                    "ss_injury_benefit": row[14]
                    and float_format(row[14])
                    or 0.0,  # total seguridad social accidente
                    "ss_sickness_benefit": row[16]
                    and float_format(row[16])
                    or 0.0,  # total seguridad social enferemedad
                }
                ...
                while (sh.row_values(line + n))[3] == "" or (
                    ...
                    if (sh.row_values(line + n))[12]:
                        record["total_accrued"] += float_format(
                            (sh.row_values(line + n))[12]
                        )
                    if (sh.row_values(line + n))[14]:
                        record["ss_injury_benefit"] += float_format(
                            (sh.row_values(line + n))[14]
                        )
                    if (sh.row_values(line + n))[16]:
                        record["ss_sickness_benefit"] += float_format(
                ...
                if record["ss_injury_benefit"]:
                    ss_total -= record[
                        "ss_injury_benefit"
                    ]  # le restamos la seguridad social por accidente
                if record["ss_sickness_benefit"]:
                    ss_total -= record[
                        "ss_sickness_benefit"
                    ]  # le resptamos la seguridad social por enfermedad

                if record["month"] and obj.month == record["month"] and year:
                    first_day, last_day = calendar.monthrange(
                        int(year), int(month)
                    )
                    id = self.env["hr.employee"].search(
                        [("glasof_code", "=", record["ref_employee"])]
                    ...
                    if id:
                        ...
                        remunerations = remuneration_obj.search(
                            [
                        ...
                        if remunerations:
                            ...
                            for remun_obj in remunerations:
                                remuneration_periods = remun_obj.get_periods_remuneration(
                                    year + "-" + month + "-01",
                                    year + "-" + month + "-" + str(last_day),
                                )
                                for period in remuneration_periods:
                                    start_date, end_date = period.split("#")
                                    for remu in remuneration_obj.browse(
                                        remuneration_periods[period]
                                    ):
                                        code = ""
                                        if remu.analytic_account_id:
                                            code = str(
                                                remu.analytic_account_id.id
                                            )
                                        elif remu.analytic_distribution_id:
                                            code = "D" + str(
                                                remu.analytic_distribution_id.id
                                            )
                                        else:
                                            continue
                                        ...
                                        if remu.with_fix_qty:
                                            fixed_old = (
                                                remu.quantity * days
                                            ) / 30  # proporción correspondiente a lso días en el periodo ej. (100*5)/30
                                            fixed += fixed_old  # sumamos el fijo de esta remuneración al fijo total
                                            if not config[str(remu.id)].get(
                                                str(code) + "-fixed"
                                            ):
                                                config[str(remu.id)].update(
                                                    {
                                                        str(code)
                                                        + "-fixed": fixed_old
                                                    }
                                        ...
                                        if remu.with_contract:
                                            rule3 = (
                                                weeks * days
                                            ) / 30.0  # proporción de días en el periodo
                                            with_contact_old = (
                                                remu.contract_hours * rule3
                                            )
                                            ...
                                            if not config[str(remu.id)].get(
                                                str(code) + "-with_contract"
                                            ):
                                                config[str(remu.id)].update(
                                                    {
                                                        str(code)
                                                        + "-with_contract": with_contact_old
                                                    }
                                                )
                                            else:
                                                config[str(remu.id)][
                                            ...
                                            with_contract_ss_old = (
                                                remu.ss_hours * rule3
                                            )  # proporción de horas de seguridad social en el periodo
                                            ...
                                            if not config[str(remu.id)].get(
                                                str(code) + "-with_ss_contract"
                                            ):
                                                config[str(remu.id)].update(
                                                    {
                                                        str(code)
                                                        + "-with_ss_contract": with_contract_ss_old
                                                    }
                                                )
                                            else:
                                                config[str(remu.id)][
                                        ...
                                        if remu.with_hour_price:
                                            ...
                                            with_price_hour_old = (
                                                remu.price
                                                * (
                                                    remu.hour_price_hours
                                                    * rule3
                                                )
                                            )  # precio_hora por proporción del total de horas de la remuneración
                                            with_price_hour += (
                                                with_price_hour_old
                                            )
                                            if not config[str(remu.id)].get(
                                                str(code) + "-with_price_hour"
                                            ):
                                                config[str(remu.id)].update(
                                                    {
                                                        str(code)
                            ...
                            for obj_time in timesheets:
                                if obj_time.analytic_id:
                                    ...
                                    if obj_time.fix_qty:
                                        ...
                                        config[str(obj_time.id) + "t"].update(
                                            {
                                                str(obj_time.analytic_id.id)
                                                + "-fixed": obj_time.fix_qty
                                            }
                                        )
                                    if obj_time.contract:
                                        with_contract += (
                                            obj_time.hours
                                        )  # sumamos a las horas totales las horas con contrato en el parte de horas
                                        # with_contract_timesheet += obj_time.hours # sumamos a las horas en parte las horas con contrato del parte
                                        config[str(obj_time.id) + "t"].update(
                                            {
                                                str(obj_time.analytic_id.id)
                                                + "-with_contract": obj_time.hours
                                            }
                                        )
                                    if obj_time.ss_hours:
                                        ss_total_hours += (
                                            obj_time.ss_hours
                                        )  # sumamos las horas al total horas con seguridad social
                                        # ss_total_timesheet += obj_time.ss_hours
                                        config[str(obj_time.id) + "t"].update(
                                            {
                                                str(obj_time.analytic_id.id)
                                                + "-with_ss_contract": obj_time.ss_hours
                                            }
                                        )

                        if (
                            journal_id
                            and general_account_id_ss
                            and general_account_id_suelsala
                            and config
                            and (fixed or with_contract or with_price_hour)
                        ):
                            for remuneration in config:
                                if (
                                    remuneration.isdigit()
                                ):  # si es una remuneración
                                    for configuration in config[remuneration]:

                                        type = (configuration.split("-"))[
                                            1
                                        ]  # tipo del importe
                                        ...
                                        analytic = (
                                            ...
                                            and int(code)
                                            or False
                                        )  # id de cuenta analítica
                                        if analytic:
                                            analytic_objs = self.env[
                                        ...
                                        for analytic_obj in analytic_objs:
                                            ...
                                            if (
                                                ...
                                                ids_delete = self.env[
                                                    ...
                                                    [
                                                        ...
                                                        (
                                                            "department_id",
                                                            "=",
                                                            analytic_obj.department_id
                                                            and analytic_obj.department_id.id
                                                            or False,
                                                        ),
                                                        (
                                                            "delegation_id",
                                                            "=",
                                                            analytic_obj.delegation_id
                                                            and analytic_obj.delegation_id.id
                                                            or False,
                                                        ),
                                                        (
                                                            "manager_id",
                                                            "=",
                                                            analytic_obj.manager_id
                                                            and analytic_obj.manager_id.id
                                                            or False,
                                                        ),
                                                    ]
                                                )  # se borrar todos los apunets ya existenetes con el mismo formato para no duplicar
                                                if ids_delete:
                                                    ids_delete.unlink()
                                                    ids_delete = []
                                                ...
                                                vals = {
                                                    ...
                                                    "department_id": analytic_obj.department_id
                                                    and analytic_obj.department_id.id
                                                    or False,
                                                    "delegation_id": analytic_obj.delegation_id
                                                    and analytic_obj.delegation_id.id
                                                    or False,
                                                    "manager_id": analytic_obj.manager_id
                                                    and analytic_obj.manager_id.id
                                                    or False,
                                                    "employee_id": hr_employee_obj.id,
                                                ...
                                                self.env[
                                                    ...
                                                    vals
                                                )  # se crear un apunte según el tipo y el importe
                                            elif (
                                                type == "with_contract"
                                                and new_valor
                                            ):  # si es con contrato y hay importe
                                                ids_delete = self.env[
                                                    "account.analytic.line"
                                                ].search(
                                                    [
                                                ...
                                                cost_fixed = (
                                                    ...
                                                    - fixed
                                                )  # restamos del total bruto la cantidad fija, obteniendo el coste no fijo
                                                cost_price_hour = (
                                                    cost_fixed
                                                    - with_price_hour
                                                )  # restamos del coste que nos queda el coste por precio hora
                                                remu_total_cost = (
                                                ...
                                                self.env[
                                                    ...
                                                    vals
                                                )  # creakos el apunte de con contrato
                                            elif (
                                                new_valor and ss_total
                                            ):  # si hay seguridad social a repartir
                                                ids_delete = self.env[
                                                    "account.analytic.line"
                                                ].search(
                                                    [
                                    ...
                                    for configuration in config[remuneration]:
                                        ...
                                        analytic_obj = self.env[
                                            "account.analytic.account"
                                        ].browse(analytic)
                                        valor = config[remuneration][
                                            configuration
                                        ]
                                        timesheet_obj_id = self.env[
                                            "timesheet"
                                        ].browse(int(remuneration[:-1]))
                                        if (
                                            type
                                            not in (
                                                "with_contract",
                                            ...
                                            vals = {
                                                ...
                                                "department_id": timesheet_obj_id.department_id
                                                and timesheet_obj_id.department_id.id
                                                or (
                                                    analytic_obj.department_id
                                                    and analytic_obj.department_id.id
                                                    or False
                                                ),
                                                "delegation_id": timesheet_obj_id.delegation_id
                                                and timesheet_obj_id.delegation_id.id
                                                or (
                                                    analytic_obj.delegation_id
                                                    and analytic_obj.delegation_id.id
                                                    or False
                                                ),
                                                "manager_id": timesheet_obj_id.responsible_id
                                                and timesheet_obj_id.responsible_id.id
                                                or (
                                                    analytic_obj.manager_id
                                                    and analytic_obj.manager_id.id
                                                    or False
                                            ...
                                            if ids_delete:
                                                ...
                                                ids_delete = []
                                            cost_fixed = (
                                                record["total_accrued"] - fixed
                                            )  # restamos del total el importe fijo
                                            cost_price_hour = (
                                                cost_fixed - with_price_hour
                                            )  # retsmoa el importe por rpecio hora
