...
class Timesheet(models.Model):
    ...
    company_id = fields.Many2one(
        ...
        default=lambda r: r._context.get('company_id',
                                         r.env.user.company_id.id))
    pending_qty = fields.Float('Quantity to Distribute', digits=(12, 2),
                               compute='_compute_pending_qty')
    pending_distribute_qty = fields.Float('Pending Quantity', digits=(12, 2),
                                          compute='_compute_pending_qty')
    ...
    employee_delegation_id = fields.Many2one('res.delegation',
                                             'Employee Delegation')
    ...
    @api.depends('extra_hours', 'price_hours', 'hours',
                 'quantity', 'effective', 'fix_qty')
    def _compute_pending_qty(self):
        for tsobj in self:
            pending_qty = (tsobj.extra_hours * tsobj.price_hours) + \
                (tsobj.hours * tsobj.price_hours) + tsobj.quantity
        ...
        if self.env.user.id != 1:
            for tim in self:
                if tim.create_uid.id != self.env.user.id:
                    raise UserError(
                        _('Error'),
                        _('Cannot delete this timesheet, '
