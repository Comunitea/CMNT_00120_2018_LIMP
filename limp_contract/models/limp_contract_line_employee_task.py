...
class LimpContractLineEmployeeTask(models.Model):
    ...
    task_ids = fields.One2many(
        "limp.contract.line.task.rel", "employee_task_line_id", "Tasks"
    )
    ...
    def create(self, vals):
        ...
        if (
            res.contract_id.sale_id
            and res.contract_id.sale_id.task_frequency_ids
        ):
            res.get_order_tasks()
        else:
            res.get_all_tasks()
    ...
    def get_order_tasks(self):
        for line in self:
            if line.contract_id.sale_id:
                task_ids = self.env["limp.contract.line.task.rel"].search(
