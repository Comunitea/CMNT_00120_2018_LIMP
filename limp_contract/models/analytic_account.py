...
class AccountAnalyticAccount(models.Model):
    ...
    concept_amount = fields.Float(
        ...
        compute="_compute_concept_amount",
    )
    ...
    is_picking = fields.Boolean(
        ...
        search="_search_is_picking",
    )
    is_picking_in_contract = fields.Boolean(
        "Is picking in contract",
        compute="_compute_is_picking_contract",
    ...
    def _process_concept_name(self, concept_rel, date):
        ...
        if cleaning_lines:
            ...
            if (
                line_cleaning_obj.address_id
                and line_cleaning_obj.address_id.name
            ):
                name = name.replace(
                    "%(center)s", line_cleaning_obj.address_id.name
                )
        return name
    ...
    def __group_by_product_lines(self, ref_line, grouped_lines):
        res = super(AccountAnalyticAccount, self).__group_by_product_lines(
            ref_line, grouped_lines
        )
    ...
    def _invoice_line_hook(self, concept, invoice_line, end_date):
        super(AccountAnalyticAccount, self)._invoice_line_hook(
        ...
        res.write(
            {
                ...
                and self.manager_id.user_id.id
                or self.env.user.id,
    ...
    def write(self, vals):
        ...
        if vals.get("state") and vals["state"] in (
            "open",
            "close",
            "cancelled",
        ):
