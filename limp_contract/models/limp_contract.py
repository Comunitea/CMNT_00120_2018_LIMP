...
class LimpContract(models.Model):
    ...
    prorogation_date = fields.Date(
        "Prorogation date", help="Estimate prorogation date"
    )
    prorogation_end_date = fields.Date(
        "Prorogation end date", help="Prorogation end date"
    )
    prorogation_notes = fields.Text("Notes")
    request_prorogation_date = fields.Date("Req. prorogation date")
    upamount_date = fields.Date(
        "Upamount date", help="Estimate up amount date"
    )
    request_upamount_date = fields.Date("Req. upamount date")
    amount = fields.Float(
        "Total annual amount",
        digits=dp.get_precision("Account"),
        compute="_compute_contract_total_amount",
    )
    ...
    bank_account_id = fields.Many2one(
        "res.partner.bank", "Bank account", help="Income bank account"
    )
    ...
    state = fields.Selection(
        [
            ("draft", "Draft"),
            ("wait_signature", "Waiting signature"),
            ("open", "Opened"),
            ("close", "Closed"),
            ("cancelled", "Cancelled"),
        ],
    ...
    home_help_line_ids = fields.One2many(
    ...
    cleaning_line_ids = fields.One2many(
        "limp.contract.line.cleaning",
        "contract_id",
        "Cleaning lines",
        states={
            "cancelled": [("readonly", True)],
    ...
    stock_service_picking_ids = fields.One2many(
    ...
    stock_sporadic_service_picking_ids = fields.One2many(
        "stock.service.picking",
        "contract_id",
        "Sporadic picking lines",
        states={
            "cancelled": [("readonly", True)],
            "close": [("readonly", True)],
        },
    ...
    payment_type_id = fields.Many2one(
        "account.payment.mode", "Payment type"
    )  # MIGRACION: Movido a payment.mode
    contact_id = fields.Many2one("res.partner", "Contact")
    note = fields.Text("Description")
    include_pickings = fields.Boolean("Include pickings")
    signature_date = fields.Date("Signature date", readonly=True)
    contract_note_ids = fields.One2many(
        "limp.contract.note", "contract_id", "Notes"
    )
    ...
    invoice_count = fields.Integer(
        string="# of Invoices", compute="_compute_invoiced", readonly=True
    )
    upamount_history_count = fields.Integer(
        string="# of upamount_history",
        compute="_compute_upamount_history_count",
        readonly=True,
    )
    home_help_lines_count = fields.Integer(
        string="# of home_help_lines",
        compute="_compute_home_help_lines_count",
        readonly=True,
    )
    cleaning_lines_count = fields.Integer(
        string="# of cleaning_lines",
        compute="_compute_cleaning_lines_count",
        readonly=True,
    )
    waste_lines_count = fields.Integer(
        string="# of waste",
        compute="_compute_waste_lines_count",
        readonly=True,
    )
    ...
    active_remuneration_lines_count = fields.Integer(
        string="# of remunerations",
        compute="_compute_active_remuneration_lines_count",
        readonly=True,
    )
    contract_note_count = fields.Integer(
        string="# of notes",
        compute="_compute_contract_note_count",
        readonly=True,
    )
    analytic_moves_count = fields.Integer(
        string="# of consummtions",
        compute="_compute_analytic_moves_count",
        readonly=True,
    )
    maintenance_task_count = fields.Integer(
        string="# of maintenance tasks",
        compute="_compute_maintenance_task_count",
        readonly=True,
    )
    timesheet_count = fields.Integer(
        string="# of timesheets",
    ...
    def _compute_contract_total_amount(self):
        for contract in self:
            ...
            for home_help_line in contract.home_help_line_ids.filtered(
                lambda x: x.state == "open"
            ):
                amount += sum(
                    home_help_line.concept_ids.mapped("total_amount")
                )
            for cleaning_line in contract.cleaning_line_ids.filtered(
                lambda x: x.state == "open"
            ):
        ...
        for contract in self:
            if vals.get("state", False) and vals["state"] in [
                ...
                contract.home_help_line_ids.filtered(
                    lambda r: r.state in ["open", "draft"]
                ).write({"state": vals["state"]})
            ...
            if vals.get("date", False):
                ...
                ).sudo().write({"date_to": vals["date"]})

            if vals.get("date_start", False):
                contract.home_help_line_ids.write(
    ...
    def onchange_partner_id(self):
        if self.partner_id:
            ...
            payment_type = (
                partner_obj.customer_payment_mode_id
                and partner_obj.customer_payment_mode_id
                or False
            )
            self.payment_type_id = payment_type and payment_type.id or False
            self.payment_term_id = (
                partner_obj.property_payment_term_id
    ...
    def act_reopen(self):
        for contract in self:
            line_ids = contract.child_ids.filtered(
                lambda x: x.date == contract.date
            )
    ...
    def action_view_upamount_history(self):
        ...
        action["domain"] = (
            "[('id','in', ["
            + ",".join(map(str, self.upamount_history_ids._ids))
            + "])]"
        )
        return action
    ...
    def _compute_waste_lines_count(self):
        for contract in self:
            contract.waste_lines_count = len(
                contract.stock_service_picking_ids
            )
    ...
    def _compute_service_picking_lines_count(self):
        for contract in self:
            contract.service_picking_lines_count = len(
                contract.stock_sporadic_service_picking_ids
            )
    ...
    def _compute_active_remuneration_lines_count(self):
        for contract in self:
            contract.active_remuneration_lines_count = len(
                contract.active_remuneration_ids
            )
    ...
    def action_view_contract_note(self):
        action = self.env.ref(
            "limp_contract.limp_contract_note_action"
        ).read()[0]
        action["context"] = str({"default_contract_id": self.id})
        action["domain"] = (
            "[('id','in', ["
            + ",".join(map(str, self.contract_note_ids._ids))
            + "])]"
        )
        return action
    ...
    def action_view_analytic_moves(self):
        action = self.env.ref(
            "analytic_material_costs.analytic_stock_move_concepts"
        ).read()[0]
        action["context"] = str(
            {
        ...
        action["domain"] = (
            "[('id','in', ["
            + ",".join(map(str, self.analytic_move_ids._ids))
            + "])]"
        )
        return action
    ...
    def _compute_maintenance_task_count(self):
        for contract in self:
            contract.maintenance_task_count = len(
                contract.maintenance_task_ids
            )

    def action_view_maintenance_task(self):
        action = self.env.ref("limp_contract.maintenance_task_action").read()[
            0
        ]
        action["context"] = str(
        ...
        action["domain"] = (
            "[('id','in', ["
            + ",".join(map(str, self.maintenance_task_ids._ids))
            + "])]"
        )
        return action
    ...
    def action_view_timesheet(self):
        action = self.env.ref(
            "limp_distribution_costs.action_timesheet"
        ).read()[0]
        action["context"] = str(
            {  # TODO: AÑADIR RESPONSABLE
                "default_delegation_id": self.delegation_id.id,
