...
class LimpContractLineHomeHelp(models.Model):
    ...
    beneficiary_amount = fields.Float(
        "Beneficiary amount", digits=dp.get_precision("Account")
    )
    check_amount = fields.Float(
        "Check amount", digits=dp.get_precision("Account")
    )
    administration_amount = fields.Float(
        "Administration amount", digits=dp.get_precision("Account")
    )
    partner_social_worker_id = fields.Many2one(
        "res.partner", "Partner social worker"
    )
    ...
    def onchange_customer_contact_id(self):
        ...
        if self.customer_contact_id:
            contact_obj = self.env["res.partner"].browse(
                self.customer_contact_id
            )
            if (
                contact_obj.job_id
                and contact_obj.job_id.address_id
                and contact_obj.job_id.address_id.state_id
            ):
                self.state_id = contact_obj.job_id.address_id.state_id.id
            if (
                contact_obj.job_id
                and contact_obj.job_id.address_id
                and contact_obj.job_id.address_id.location
            ):
                self.location_id = contact_obj.job_id.address_id.location.id
            if (
                contact_obj.job_id
                and contact_obj.job_id.address_id
                and contact_obj.job_id.address_id.region
            ):
                self.region_id = contact_obj.job_id.address_id.region.id
            if (
                contact_obj.job_id
                and contact_obj.job_id.address_id
                and contact_obj.job_id.address_id.council_id
            ):
