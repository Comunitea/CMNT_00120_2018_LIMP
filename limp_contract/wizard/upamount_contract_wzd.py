...
class UpamountContractWzd(models.TransientModel):
    ...
    def _reg_upamount(self, contract, upamount_percent, previous_amount, new_amount, name):
    ...
    def upamount_action(self):
        ...
        for contract in contracts:
            #Annual amount
            if contract.amount:
                new_amount = contract.amount + (contract.amount * (self.upamount_percent / 100.0))
                contract.write({'amount': new_amount})
                self._reg_upamount(contract, self.upamount_percent, contract.amount, new_amount, contract.name + _(u": Annual amount"))

            #contract invoice concepts
            self._update_concepts(contract.analytic_account_id, self.upamount_percent, contract)

            for home_line in contract.home_help_line_ids:
                self._update_concepts(home_line.analytic_acc_id, self.upamount_percent, contract)
            for clean_line in contract.cleaning_line_ids:
                self._update_concepts(clean_line.analytic_acc_id, self.upamount_percent, contract)

            contract.write({'upamount_date': (datetime.now() + relativedelta(years=+1)).strftime("%Y-%m-%d")})
