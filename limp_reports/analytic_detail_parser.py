...
class AccountAnalyticDetails(models.AbstractModel):
    ...
    def _get_balance(self, account, data, with_tag_accounts=False):
        ...
        self._cr.execute(
            """select sum(amount),count(*)
               from account_analytic_line where {}""".format(
                domain
            )
        )
    ...
    def _get_journals(
        self, account, data, child=False, with_tag_accounts=False
    ):
        domain = self._get_domain(
            account, data, with_tag_accounts=with_tag_accounts
        )
        ...
        for employee_id in employee_ids:
            ...
            if employee_id:
                ...
                self._cr.execute(
                    ...
                    select sum(amount)
                    from account_analytic_line
                    where {} and employee_id is null""".format(
                        domain
                    )
                )
            balance = self._cr.fetchone()[0]
    ...
    def _get_analytic_type(
        self, account, tag, employee, data, with_tag_accounts=False
    ):
        res = []
        domain = self._get_domain(
            account, data, with_tag_accounts=with_tag_accounts
        ...
        for type_n in type_names:
            if type_n:
                ...
                self._cr.execute(
                    ...
                    select sum(amount)
                    from account_analytic_line
                    where {} and type_analytic is null""".format(
                        domain
                    )
                )
            balance = self._cr.fetchone()[0]
