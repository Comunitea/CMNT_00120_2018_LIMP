...
class StockPicking(models.Model):
    ...
    waste_or_arid = fields.Selection([('waste', 'Wastes'), ('arid', 'Arid')], 'Waste/Arid', default='waste')
    manager_or_productor = fields.Selection([('manager', 'Manager'), ('productor', 'Productor')], 'Manager/Productor', default='productor')
    ...
    def action_invoice_create(self, journal_id, group, date):
        ...
        for key in grouped_pickings:
            ...
            updates = self.env['account.invoice'].onchange(invoice_vals, ['partner_id'], specs)
            ...
            for operation in grouped_pickings[key].mapped('pack_operation_product_ids'):
                invoice_line_vals = {
                ...
                updates = self.env['account.invoice.line'].onchange(invoice_line_vals, ['product_id'], specs)
                value = updates.get('value', {})
                for name, val in value.iteritems():
                    if isinstance(val, tuple):
                        value[name] = val[0]
        ...
        for pick in self:
                ...
                if move.product_id.ler_code_id and pick.picking_type_id.code == 'outgoing':
                    ...
                    if orig_qty < 0.0 and move.product_id.id not in visited_products:
                        ...
                        if not self.env.user.company_id.reserve_product_id:
                            raise UserError(_(u'You have not enough quantity to serve the product %s. Then you have to set teh reserve product in company for using it.') % (move.product_id.name,))
                        else:
                            ...
                            reserve_moves1 += self.env['stock.move'].create(move_vals)
            ...
            if reserve_moves2:
                ...
                if pick_2.state != 'assigned':
                    ...
                    if pick_2.state != 'assigned':
                        pick_2.force_assign()
                self.env['stock.immediate.transfer'].create({'pick_id': pick_2.id}).process()
