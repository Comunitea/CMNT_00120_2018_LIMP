...
class StockServicePickingLine(models.Model):
    ...
    name = fields.Char(
        ...
        default=lambda r: r._context.get("parent_name", False),
    )
    ...
    delivery_kms = fields.Integer(
        "Delivery kms.", states={"done": [("readonly", True)]}
    )
    arrival_kms = fields.Integer(
        "Arrival kms.", states={"done": [("readonly", True)]}
    )
    delivery_hours = fields.Float(
        "Delivery hours", digits=(4, 2), states={"done": [("readonly", True)]}
    )
    displacement_hours = fields.Float(
        "Displacement hours",
        digits=(4, 2),
        states={"done": [("readonly", True)]},
    )
    work_hours = fields.Float(
        "Work hours", digits=(4, 2), states={"done": [("readonly", True)]}
    )
    tranfer_hours = fields.Float(
        "Transfer hours", digits=(4, 2), states={"done": [("readonly", True)]}
    )
    arrival_hours = fields.Float(
        "Arrival hours", digits=(4, 2), states={"done": [("readonly", True)]}
    )
    ...
    transport_type = fields.Char(
        "Transport type", size=128, states={"done": [("readonly", True)]}
    )
    picking_id = fields.Many2one(
        "stock.service.picking", "Picking", required=True, readonly=True
    )
    ...
    orig_address_id = fields.Many2one(
        "res.partner", "Origin address", states={"done": [("readonly", True)]}
    )
    dest_address_id = fields.Many2one(
        "res.partner", "Dest. address", states={"done": [("readonly", True)]}
    )
    transport_form = fields.Selection(
        [("tub", "Tub"), ("container", "Container"), ("other", "Other")],
    ...
    other_type_text = fields.Char(
        "Type description", size=255, states={"done": [("readonly", True)]}
    )
    carrier_id = fields.Many2one(
        "res.partner", "Carrier", states={"done": [("readonly", True)]}
    )
    other_carrier = fields.Char(
        "Other carrier", size=128, states={"done": [("readonly", True)]}
    )
    ...
    driver_id = fields.Many2one(
        "hr.employee", "Driver", states={"done": [("readonly", True)]}
    )
    ...
    parent_company_id = fields.Many2one(
        ...
        default=lambda r: r._context.get(
            "parent_company_id", r.env.user.company_id.id
        ),
    )
    parent_man_addr_id = fields.Many2one(
        "res.partner",
        "Manager Address",
        default=lambda r: r._context.get("parent_man_addr_id", False),
    )
    parent_building_addr_id = fields.Many2one(
        "res.partner",
        "Building Parent Address",
        default=lambda r: r._context.get("parent_building_addr_id", False),
    )
    total_hours = fields.Float(
        "Total hours", digits=(16, 2), compute="_compute_total_hours"
    )
    ...
    def action_reopen(self):
        for line in self:
            if line.container_id and line.dest_address_id:
                container_move_ids = self.env["container.move"].search(
                    [("container_id", "=", line.container_id.id)], limit=2
                )
                if container_move_ids:
                    ...
                    if move.move_type == "in":
                        ...
                        if (
                            line.container_id.situation_id
                            and line.container_id.situation_id.id
                            == line.dest_address_id.id
                        ):
