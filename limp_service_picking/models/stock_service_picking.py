...
class StockServicePicking(models.Model):
    ...
    manager_partner_id = fields.Many2one(
        "res.partner",
        "Manager",
        help="Destination partner",
        states={
            "closed": [("readonly", True)],
            "cancelled": [("readonly", True)],
        },
    )
    manager_address_id = fields.Many2one(
        "res.partner",
        "Address",
    ...
    producer_mark = fields.Selection(
        [("small_producer", "Small producer"), ("producer", "Producer")],
        "Producer type",
        states={
            "closed": [("readonly", True)],
            "cancelled": [("readonly", True)],
    ...
    ccc_account_id = fields.Many2one(
        "res.partner.bank",
        "Bank",
    ...
    print_service_ids = fields.One2many(
        "stock.service.picking.line",
        "picking_id",
        "Transports to print",
        readonly=True,
        domain=[("no_print", "=", False)],
        copy=False,
    )
    move_service_ids = fields.One2many(
        "stock.service.picking.line",
        "picking_id",
        "Move transports",
        readonly=True,
    ...
    remove_service_ids = fields.One2many(
        "stock.service.picking.line",
        "picking_id",
        "Remove transports",
        readonly=True,
    ...
    ler_dangerous = fields.Boolean(
        related="ler_code_id.dangerous", readonly=True, string="Dangerous"
    )
    gross_weight = fields.Float(
        "Gross (kg.)",
        digits=(12, 2),
        help="Gross weight in Kg",
        states={
            "closed": [("readonly", True)],
            "cancelled": [("readonly", True)],
        },
    ...
    container_situation_id = fields.Many2one(
        "res.partner",
        related="container_id.situation_id",
        readonly=True,
        string="Container situation",
        copy=False,
    )
    dcs_no = fields.Char(
        "DCS no.",
    ...
    holder_partner = fields.Char(
        "Holder / Builder",
        size=255,
        states={"cancelled": [("readonly", True)]},
    )
    holder_address = fields.Char(
        "Address", size=148, states={"cancelled": [("readonly", True)]}
    )
    producer_partner = fields.Char(
        "Producer", size=255, states={"cancelled": [("readonly", True)]}
    )
    producer_address = fields.Char(
        "Producer Address",
        size=148,
        states={"cancelled": [("readonly", True)]},
    )
    building_site_address_id = fields.Many2one(
        "res.partner",
        "Building site address",
        states={"cancelled": [("readonly", True)]},
    )
    building_site_city = fields.Char(
        "City", size=128, states={"cancelled": [("readonly", True)]}
    )
    ...
    building_site = fields.Boolean(
        "Building site",
        help="Demolition and building waste",
        states={
            "closed": [("readonly", True)],
            "cancelled": [("readonly", True)],
        },
    ...
    invoice_date = fields.Date(
        "Invoice date", states={"cancelled": [("readonly", True)]}, copy=False
    )
    ...
    fiscal_position = fields.Many2one(
        ...
        "Fiscal position",
        states={"cancelled": [("readonly", True)]},
        default=lambda r: r._context.get("fiscal_position", False),
        copy=False,
    )
    payment_term = fields.Many2one(
        ...
        "Payment term",
        states={"cancelled": [("readonly", True)]},
        default=lambda r: r._context.get("payment_term", False),
        copy=False,
    )
    ...
    service_invoice_concept_ids = fields.One2many(
        "service.picking.invoice.concept",
        "service_picking_id",
        "Invoice concepts",
        copy=False,
    )
    other_concepts_ids = fields.One2many(
        "service.picking.other.concepts.rel",
        "service_picking_id",
        "Other concepts",
        copy=False,
    )
    ...
    name_manager = fields.Char(
        ...
        default=lambda r: r.env.user.company_id.partner_id.name,
        copy=False,
    )
    authorization_manager = fields.Char(
    ...
    picking_type = fields.Selection(
        ...
        "Type",
        change_default=True,
        default=lambda r: r._context.get("type", "wastes"),
    )
    service_picking_valorization_ids = fields.One2many(
        "service.picking.valorization.rel",
        "service_picking_id",
        "Valorization",
        states={"cancelled": [("readonly", True)]},
        copy=False,
    ...
    product_tax_id = fields.Many2one(
        "product.product", "Product tax", copy=False
    )
    sand_amount = fields.Float("Amount", digits=(16, 2), copy=False)
    product_sand_id = fields.Many2one(
        "product.product", "Sand Treatment", copy=False
    )
    ...
    invoice_delegation_id = fields.Selection(
        ...
        string="Delegation",
        help="Delegation where inputs the cost",
    )
    invoice_department_id = fields.Many2one(
        "hr.department", "Department", help="Department where inputs the cost"
    )
    invoice_responsible_id = fields.Many2one(
        "hr.employee",
        "Responsible",
        help="Responsible who inputs the cost",
        domain=[("responsible", "=", True)],
    )
    delivery_proof_no = fields.Char(
        "Delivery proof no.", size=128, readonly=True, copy=False
    )
    not_print_acceptation = fields.Boolean(
        "Not print acceptation",
        help="Not print acceptation in delivery waste check report",
    )
    ...
    def onchange_partner_id(self):
        ...
        self.ccc_account_id = (
            (payment and payment.suitable_bank_types and part.bank_ids)
            and part.bank_ids[0].id
            or False
        )
        self.fiscal_position = (
            part.property_account_position_id
            and part.property_account_position_id.id
            or False
        )
        self.payment_term = (
            part.property_payment_term_id
            and part.property_payment_term_id.id
            or False
        )
    ...
    def onchange_manager_partner_id(self):
        ...
        self.authorization_manager = (
            self.manager_partner_id.manager_authorization_no
            and self.manager_partner_id.manager_authorization_no
            or False
        )
    ...
    def action_active(self):
        ...
        for picking in self:
            for service in picking.type_ddd_ids:
                if service.service_type == "ddd" and not picking.n_cert_ddd:
                    picking.n_cert_ddd = self.env["ir.sequence"].next_by_code(
                        "treatment.certificate.ddd"
                    )
                elif (
                    service.service_type == "legionella" and not picking.n_cert
                ):
    ...
    def create_concept_lines(self):
        for order in self:
            ...
            for waste in order.service_picking_valorization_ids:
                ...
                if waste.overload_qty and (
                    waste.product_id and waste.product_id.overload_price
                ):
                    vals = {
                        "product_id": waste.product_id.id,
                        "sequence": seq,
        ...
        for order in self:
            ...
            if order.picking_type == "wastes" and not order.retired_date:
                ...
                for line in order.remove_service_ids:
                    if (
                        not retired_date
                        or retired_date < line.transport_date[:10]
                    ):
