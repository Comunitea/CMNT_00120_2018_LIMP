...
class ServiceOrderToinvoice(models.TransientModel):
    ...
    def create_invoice(self):
        ...
        for service_picking in self.env["stock.service.picking"].browse(
            ...
            intercompany = False
            if (
                service_picking.invoice_line_ids
                or service_picking.state != "closed"
                or service_picking.invoice_type == "noinvoice"
            ):
                continue
            key = (
                "del:"
                + str(service_picking.delegation_id.id)
                + "/dep:"
                + str(service_picking.department_id.id)
                + "/m:"
            ...
            if (
                ...
                group_partner
                and (key + "p:" + str(partner.id)) in invoices_group
            ):
            ...
            if intercompany:
                in_journal_ids = self.env["account.journal"].search(
                    [("intercompany", "=", True)]
                )
                ...
                for line in invoice_copied.invoice_line_ids:
                    ...
                    if not account_id:
                        raise UserError(
                            _("Expense account in product %s is not set")
