...
class AccountBalanceXls(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        sheet.write(
            3, 14, round(amount / (months_with_results or 1.0), 2), cell_format
        )
        sheet.write(
            3, 15, round(amount / (months_with_results or 1.0), 2), cell_format
        )
        ...
        for tag in tags:
            ...
            for month in months:
                ...
                if month_invoice_amouonts[month[0]]:
                    percent = round(
                        (unit_amount * 100.0)
                        / month_invoice_amouonts[month[0]],
                        2,
                    )
                else:
                    percent = 0
            ...
            if average_percent > target_percent:
                ...
                if diff > 1.5:
                    cell_format = workbook.add_format(
                        merge_dicts(
                            cell_styles["very_very_far_values"],
                            cell_styles["bold"],
                            cell_styles["bordered"],
                        )
                    )
        ...
        cell_format = workbook.add_format(
            merge_dicts(
                cell_styles["target_label_font"], cell_styles["bordered"]
            )
        )
