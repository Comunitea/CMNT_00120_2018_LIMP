...
class AccountAnalyticAccount(models.Model):
    ...
    concept_ids = fields.One2many('account.analytic.invoice.concept.rel',
                                  'analytic_id', 'Concepts')
    group_concepts = fields.Boolean('Group concepts',
                                    help='Groups concepts at invoice')
    group_products = fields.Boolean('Group products',
    ...
    def __group_by_product_lines(self, ref_line, grouped_lines):
        ...
        ref_line.write(
            {'price_unit': round(subtotal, 2),
             'name':
                 ref_line.product_id.description_sale or
                 ref_line.product_id.name,
                 'note': note})
        return True
    ...
    def run_invoice_cron_manual(self):
        ...
        analytic_ids = self.env['account.analytic.account'] # list of visited analytic accounts
        created_invoices = self.env['account.invoice'] # list of created invoices
            ...
            if analytic_account not in analytic_ids:
                if analytic_account.group_concepts and \
                    ...
                    for analytic_child_obj in child_ids:
                        ...
                        for child_concept in analytic_child_obj.concept_ids: #goes around child concepts
                            res = analytic_child_obj.create_concept_invoice_line(child_concept, invoice, end_date) #creates an invoice line for each child concept
                            if res:
                                child_concept.write(
                                    {'last_invoice_date': end_date})
                        analytic_ids += analytic_child_obj  #set account as visited
                elif analytic_account.concept_ids or child_concepts_ids: #if exists concepts buy they don't group and it isn't visited
                    invoice = analytic_account._create_invoice(end_date) #invoice by concept
                    created_invoices += invoice
                    analytic_invoices += invoice
                    for concept in analytic_account.concept_ids:
                        res = analytic_account.create_concept_invoice_line(
                            concept, invoice, end_date) #invoice line by conept
                        if res:
                            concept.write({'last_invoice_date': end_date})
                ...
                if analytic_account.group_products:
                    for inv in analytic_invoices:
                        ...
                        for key_id in dicc:
                            to_delete_line_ids = self.env['account.invoice.line']
                            if len(dicc[key_id]) > 1:
                                ...
                                self.__group_by_product_lines(ref_line, to_delete_line_ids)
                                new_name = self.env['account.analytic.invoice.concept'].process_name(
