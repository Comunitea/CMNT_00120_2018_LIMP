...
class invoice_line_agent(models.Model):
    ...
    def calculate_commission(self, cr, uid, ids):
        for line_agent in self.browse(cr, uid, ids):
            if line_agent.commission_id.type == 'fijo' and line_agent.commission_id.fix_percent:
                quantity = line_agent.invoice_line_id.price_subtotal * (line_agent.commission_id.fix_percent / 100.0)
                self.write(cr, uid, line_agent.id, {'quantity': quantity})
            elif line_agent.commission_id.type == 'fijo' and line_agent.commission_id.fix_qty:
                quantity = line_agent.commission_id.fix_qty
                self.write(cr, uid, line_agent.id, {'quantity': quantity})
    ...
    def onchange_agent_id(self, cr, uid, ids, agent_id):
        ...
        if agent_id:
            ...
            if agent_line:
                if agent.commission.fix_percent:
                    v['quantity'] = agent_line[0].invoice_line_id.price_subtotal * (agent.commission.fix_percent / 100.0)
                elif agent.commission.fix_qty:
                    v['quantity'] = agent.commission.fix_qty
            else:
                v['quantity'] = 0
    ...
    def onchange_commission_id(self, cr, uid, ids, agent_id, commission_id):
        ...
        if commission_id:
            ...
            agent_line = self.browse(cr, uid, ids)
            if partner_commission.fix_percent:
                v['quantity'] = agent_line[0].invoice_line_id.price_subtotal * (partner_commission.fix_percent / 100.0)
            elif partner_commission.fix_qty:
                v['quantity'] = partner_commission.fix_qty
            result['value'] = v
