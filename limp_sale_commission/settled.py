        ...
        for settlement in self.browse(cursor, user, ids, context=context):
            ...
            if not partner:
                for invoice in settlement.invoices:
                    data = {
                        'name' : u'Comision ' + invoice.invoice_number + u": " + settlement.agent_id.employee_id.name,
                        'employee_id' : settlement.agent_id.employee_id.id,
                        'analytic_id' : invoice.account_analytic_id and invoice.account_analytic_id.id or False,
                        'pending_qty': invoice.settled_amount,
    ...
    def calcula(self, cr, uid, ids):
        ...
        for commission in line.invoice_line_id.commission_ids:  # Recorre los agentes y condiciones asignados a la factura
            if commission.agent_id.id == line.settlement_agent_id.agent_id.id: # selecciona el asignado al agente para el que está liquidando
                ...
                if commission_app.type=="fijo" and commission_app.fix_percent:
                    commission_per = commission_app.fix_percent
                    amount = amount + line.invoice_line_id.price_subtotal * float(commission_per) / 100
                elif commission_app.type=="fijo" and commission_app.fix_qty:
                    amount = amount + commission_app.fix_qty
...
class settled_invoice_agent(osv.osv):
    ...
    _columns = {
        ...
        'account_analytic_id': fields.many2one('account.analytic.account', 'Analytic acc.', readonly=True, select=1),
    ...
    def init(self, cr):
        ...
        cr.execute("""
            ...
            account_invoice_line.invoice_id as invoice_id, settlement_agent.agent_id as agent_id, account_invoice_line.account_analytic_id as account_analytic_id,
