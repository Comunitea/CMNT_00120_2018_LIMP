...
class EmployeeReplacementWzd(models.TransientModel):
    ...
    def _get_default_employee(self):
        ...
        if remuneration_ids:
            remuneration_obj = self.env["remuneration"].browse(
                remuneration_ids[0]
            )
    ...
    search_employee_id = fields.Many2one(
        "hr.employee", default=_get_default_employee
    )
    search_location = fields.Many2one(
        "city.council", "Council", default=_get_default_location
    )
    search_department_id = fields.Many2one(
        "hr.department", "Department", default=_get_default_department
    )
    ...
    def action_replace(self):
        ...
        if remuneration_ids:
            ...
            for line_remuneration in remuneration_objs:
                ...
                if (
                    self.conditions != "equal_condition"
                    and not self.distribute_bt_remuneration
                ):
                    ...
                    ss_no_hours = (
                        self.ss_no_hours * line_remuneration.ss_no_hours
                ...
                type_incidence_id = self.env.ref(
                    "analytic_incidences.incidence_replacement"
                ).id
