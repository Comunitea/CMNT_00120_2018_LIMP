...
<form string="Zip codes">
    ...
    <label string="This allows to fill automatically the region field of partner and contact forms from the zip code or city." colspan="4" align="0.0"/>
</form>'''
...
class l10n_ES_create_zipcode_region(wizard.interface):
    def _create_zipcode(self, cr, uid, data, context):
        ...
        for m in cod_postales:
            ...
            state_ids = pool.get('res.country.state').search(cr, uid, [('code','=',codi)])
            if ids and state_ids:
                pool.get('res.country.state').write(cr, uid, state_ids, {'region_id': ids[0]})
        return {}
